    	   ...
    	   <div class="row">
                ...
                <div class="col-md-4 col-sm-6 col-xs-12 col-custom">
                   <div class="info-box info-box-new-style">
                        <span class="info-box-icon text-info"><i class="ion ion-ios-cart-outline"></i></span>
    	    ...
    	    <!-- /.col -->
          	...
          	<div class="row">
                <div class="col-md-4 col-sm-6 col-xs-12 col-custom">
                   <div class="info-box info-box-new-style">
                        <span class="info-box-icon text-info"><i class="ion ion-cash"></i></span>
        ...
        <!-- end is_admin check -->
         @if(auth()->user()->can('sell.view') || auth()->user()->can('direct_sell.view'))
            @if(!empty($all_locations))
              	...
              	<div class="row">
              		<div class="col-sm-12">
                        @component('components.widget', ['class' => 'box-warning', 'title' => __('home.sells_last_30_days')])
            ...
            @if(!empty($all_locations))
              	<div class="row">
              		<div class="col-sm-12">
                        @component('components.widget', ['class' => 'box-warning', 'title' => __('home.sells_current_fy')])
        ...
        <div class="row">
            <div class="@if((session('business.enable_product_expiry') != 1) && auth()->user()->can('stock_report.view')) col-sm-12 @else col-sm-6 @endif">
                @component('components.widget', ['class' => 'box-warning'])
                  ...
                  @endslot
                  @slot('title')
                    {{ __('messages.alert_cheque_client') }} @show_tooltip(__('tooltip.product_stock_alert'))
                  @endslot
                  <div class="row">
                        @if(count($all_locations) > 1)
                            <div class="col-md-6 col-sm-6 col-md-offset-6 mb-10">
                        ...
                        @endif
                        <div class="col-md-12">
                            <table class="table table-bordered table-striped" id="cheque_client_report_table" style="width: 100%;">
                                <thead>
                                  <tr>
            ...
            <div class="row">
                <div class="@if((session('business.enable_product_expiry') != 1) && auth()->user()->can('stock_report.view')) col-sm-12 @else col-sm-6 @endif">
                    @component('components.widget', ['class' => 'box-warning'])
                      ...
                      @endslot
                      @slot('title')
                        {{ __('messages.alert_cheque_fournisseur') }} @show_tooltip(__('tooltip.product_stock_alert'))
                      @endslot
                      <div class="row">
                            @if(count($all_locations) > 1)
                                <div class="col-md-6 col-sm-6 col-md-offset-6 mb-10">
                            ...
                            @endif
                            <div class="col-md-12">
                                <table class="table table-bordered table-striped" id="cheque_Fournisseur_report_table" style="width: 100%;">
                                    <thead>
                                      <tr>
                                            <th>@lang('role.supplier')</th>
                                            <th>@lang('messages.Cheque_Echeance')</th>
                                            <th>@lang('lang_v1.cheque_no')</th>
                                            <th>@lang('messages.Factur_Number')</th>
                                            <th>@lang('messages.Payment_Number')</th>
                        ...
                        <div class="row">
                            <div class="@if((session('business.enable_product_expiry') != 1) && auth()->user()->can('stock_report.view')) col-sm-12 @else col-sm-6 @endif">
                                @component('components.widget', ['class' => 'box-warning'])
                                  ...
                                  @endslot
                                  @slot('title')
                                    {{ __('messages.alert_cheque_deponse')  }} @show_tooltip(__('tooltip.product_stock_alert'))
                                  @endslot
                                  <div class="row">
                                        @if(count($all_locations) > 1)
                                            <div class="col-md-6 col-sm-6 col-md-offset-6 mb-10">
                                        ...
                                        @endif
                                        <div class="col-md-12">
                                            <table class="table table-bordered table-striped" id="cheque_Deponse_report_table" style="width: 100%;">
                                                <thead>
                                                  <tr>
...
@section('javascript')
    ...
    <script type="text/javascript">
        ...
        // cheque client
        var cheque_client_report_table = $('#cheque_client_report_table').DataTable({
    processing: true,
    serverSide: false,
    aaSorting: [[2, 'desc']],
    ajax: {
        "url": '{{action([\App\Http\Controllers\HomeController::class, 'getChequeClient'])}}',
    },
    columns: [
        { data: 'customer', name: 'c.name', searchable: true },
        { data: 'chequeEcheance', name: 'chequeEcheance', searchable: true },
        { data: 'chequeNumber', name: 'chequeNumber', searchable: true },
        { data: 'facturNumber', name: 'facturNumber', searchable: true },
        { data: 'payment_no', name: 'payment_no', searchable: true },
        { data: 'amount', name: 'amount', searchable: true },
        { data: 'chequeStatus', name: 'chequeStatus', searchable: true },
        { data: 'NumberJour', name: 'NumberJour', searchable: true },
        { data: 'action', searchable: false }
    ]
});

   // cheque Fournisseur 
var cheque_fournisseur_table = $('#cheque_Fournisseur_report_table').DataTable({
    processing: true,
    serverSide: false,
    aaSorting: [[2, 'desc']],
    ajax: {
        "url": '{{action([\App\Http\Controllers\HomeController::class, 'getChequeFournisseur'])}}',
    },
    columns: [
        { data: 'customer', name: 'c.name', searchable: true },
        { data: 'chequeEcheance', name: 'chequeEcheance', searchable: true },
   ...
   // cheque deponse
   var cheque_Deponse_table = $('#cheque_Deponse_report_table').DataTable({
    processing: true,
    serverSide: false,
    aaSorting: [[2, 'desc']],
    ajax: {
        "url": '{{action([\App\Http\Controllers\HomeController::class, 'getChequeDeponse'])}}',
    },
    columns: [
        { data: 'expense', name: 'e.name', searchable: true },
        { data: 'chequeEcheance', name: 'chequeEcheance', searchable: true },
        { data: 'chequeNumber', name: 'chequeNumber', searchable: true },
        { data: 'facturNumber', name: 'facturNumber', searchable: true },
        { data: 'payment_no', name: 'payment_no', searchable: true },
