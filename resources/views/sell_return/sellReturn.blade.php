...
<section class="content no-print">
    ...
    @slot('tool')
    <div class="box-tools">
        <a class="btn btn-block btn-primary" href="{{action([\App\Http\Controllers\SellReturnController::class, 'create'])}}">
            <i class="fa fa-plus"></i> @lang('messages.add')</a>
    </div>
    @endslot
    <div class="table-responsive">
        <table class="table table-bordered table-striped ajax_view" id="sell_return_tab">
            <thead>
                <tr>
...
<script>
    $(document).ready(function() {
        ...
        sell_return_tab = $('#sell_return_tab').DataTable({
            processing: true
            , serverSide: true
            , aaSorting: [
                [0, 'desc']
            ...
            , "fnDrawCallback": function(oSettings) {
                var total_sell = sum_table_col($('#sell_return_tab'), 'final_total');
                $('#footer_sell_return_total').text(total_sell);

                $('#footer_payment_status_count_sr').html(__sum_status_html($('#sell_return_tab'), 'payment-status-label'));

                var total_due = sum_table_col($('#sell_return_tab'), 'payment_due');
                $('#footer_total_due_sr').text(total_due);

                __currency_convert_recursively($('#sell_return_tab'));
            }
