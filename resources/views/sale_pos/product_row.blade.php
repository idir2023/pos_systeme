...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	...
	@if($price_derniere)
	<div id='price_derniere_container' style="font-size: 10px;">
	@lang('lang_v1.dernier_prix_de_vente') :<span class='text text-red'>{{ number_format($price_derniere, 2, '.', ',') }} DH</span>
	</div>
	@endif

	@if(!empty($is_direct_sell))	
	@if($product->product_custom_field1)
	@lang('lang_v1.prix_bas_vente'):<span class='text text-red'>{{$product->product_custom_field1}}</span>
	@endif
	@endif
