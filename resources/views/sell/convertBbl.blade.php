			...
			@component('components.widget', ['class' => 'box-solid'])
				...
				<div class="col-sm-3">
					<div class="form-group">
						...
						{!! Form::text('invoice_no', null, ['class' => 'form-control', 'placeholder' => $transaction->type == 'sales_order' ? __('restaurant.order_no'): __('sale.invoice_no')]); !!}
					</div>
				</div>
				@endcan
				{{--  --}}
				@if((empty($transaction->status) || (!in_array($transaction->status, ['quotation', 'draft'])) || $is_enabled_download_pdf) && $transaction->type != 'sales_order')
				...
				<div class="col-sm-3">
					<div class="form-group">
						<div class="checkbox">
							<label>
								<br>
								{!! Form::checkbox('only_facture', 1, $transaction->facture_numero ? true : false, ['class' => 'input-icheck', 'id' => 'only_facture']) !!} {{ __('messages.facture')}}
							</label>
						</div>
					</div>
				</div>
				
				<div class="col-sm-3 {{ $transaction->facture_numero ? '' : 'hide' }}" id="facture_numero_container">
					<div class="form-group">
						{!! Form::label('facture_numero', __('lang_v1.Facture_Numero') . ':') !!}
						{!! Form::text('facture_numero', $transaction->facture_numero, ['class' => 'form-control',
						'data-facture_numero' => $transaction->facture_numero,
						'placeholder' => __('lang_v1.Facture_Numero')]) !!}
					</div>
				</div> 
				@endif 
				@endif    
