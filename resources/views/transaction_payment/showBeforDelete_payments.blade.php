    ...
    <div class="modal-content">
      ...
      <h3 class="text text-danger col-md-12" style="text-align: center;"> Supprimer les paiments en premier temps</h5>
        <div class="modal-header">
            <button type="button" id="closeForm" class="close no-print" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                            ...
                            <tr>
                              ...
                              <td class="no-print" style="display: flex;">
                                ...
                                @if((auth()->user()->can('delete_purchase_payment') && (in_array($transaction->type, ['purchase', 'purchase_return']))) || (auth()->user()->can('delete_sell_payment') && (in_array($transaction->type, ['sell', 'sell_return']))) || ((auth()->user()->can('all_expense.access') || auth()->user()->can('view_own_expense')) && $transaction->type == 'expense') )
                                    &nbsp; <button type="button" id="delete_payment" class="btn btn-danger btn-xs delete_payment1" 
        ...
        <div class="modal-footer">
            <button type="button" id="closeButton" class="btn btn-default no-print" data-dismiss="modal">@lang( 'messages.close' )</button>
        </div>
    </div><!-- /.modal-content -->
</div><!-- /.modal-dialog -->
...
<script>
   /* $(document).ready(function() {
        ...
        function reloadPage() {
         ...
         $('#closeButton, #closeForm').on('click', function() {
            // Recharger la page
            reloadPage();
        });
    });*/
