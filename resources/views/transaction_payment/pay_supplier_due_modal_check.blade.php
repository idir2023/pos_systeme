          ...
          {{-- START CODE  --}}
                <div class="col-md-4">
                    <div class="form-group">
                        ...
                        {!! Form::hidden('is_advance') !!}
                        <span id="amount-alert" class="text text-red" style="display: none;">
                            Vous ne pouvez pas payer plus que le montant requis.
                        </span>

                    </div>
                </div>

                @if (session()->get('business.active_avance'))
                    <input type="hidden" id="active_avance" value="1" data-active_avance="1">
                @endif
...
<script>
  // /////////////////////////////////////////////////////////////////////////////// START CODE 
    $(document).ready(function() {
        var defaultAmount = parseFloat('{{ $payment_line->amount }}');

        if ($('#active_avance').data('active_avance') === 1) {
            $('.payment_amount').on('change', function() {
                var enteredAmount = parseFloat($(this).val());
                if (enteredAmount > defaultAmount) {
                    var difference = enteredAmount - defaultAmount;
                    var confirmation = confirm("Êtes-vous sûr de vouloir payer " + difference +
                        " une avance ?");
                    if (!confirmation) {
                        $(this).val(defaultAmount);
                    }
                }
            });
        } else {
    ...
    // /////////////////////////////////////////////////////////////////////////////// END CODE
   $(document).ready(function() {
        // Assuming your input has the name "amount" and the checkbox has the name "is_advance"
        var amountInput = $('input[name="amount"]');
        var isAdvanceInput = $('input[name="is_advance"]');
        
        // Capture the initial value when the page is loaded
        var firstValue = parseFloat(amountInput.val());

        amountInput.on('input', function() {
            var enteredValue = parseFloat($(this).val());

            // Set the boolean value for is_advance based on the condition
            var isAdvance = enteredValue > firstValue;

            // Update the hidden input value using jQuery
            isAdvanceInput.val(isAdvance ? 'is_advance' : '');
        });
    });
    $(document).ready(function () {
        // Attach an event listener to the amount input field
        $('.payment_amount').on('input', function () {
            // Get the entered amount and the payment method
            var enteredAmount = parseFloat($(this).val());
            var paymentMethod = $('select[name="method"]').val(); // Adjust the selector based on your actual payment method select field

            // Get the cheque status input field
            var chequeStatusInput = $('select[name="cheque_status"]');

            // Check if the amount is greater than $payment_line->amount and the payment method is cheque
            if (enteredAmount > {{ $payment_line->amount }} && paymentMethod === 'cheque') {
                // Set the cheque status to "encaisse"
                
                //$payment_line->cheque_status='Encaissé';
                chequeStatusInput.val('Encaissé').change().prop('disabled', true);
                $('<input>').attr({
                    type: 'hidden',
                    name: 'cheque_status',
                    value: 'Encaissé'
                ...
                // Reset the cheque status to its default value and enable the payment method select field
                chequeStatusInput.val('En cours').change().prop('disabled', false);
                $('input[name="cheque_status"]').remove();
            }
        });
    });
</script>