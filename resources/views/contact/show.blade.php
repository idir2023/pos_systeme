                        ...
                        @if (in_array($contact->type, ['both', 'customer']))
                            ...
                            @else
                                ...
                                <a href="#sales_tab" data-toggle="tab" aria-expanded="true"><i
                                        class="fas fa-arrow-circle-up" aria-hidden="true"></i> @lang('sale.sells_BL_FC')</a>
                            </li>
                            ...
                            @else
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        {{-- start purchase button regle --}}
                                        @if ($contact->type == 'supplier' || $contact->type == 'both')
                                            <div class="clearfix"></div>
                                            @if ($contact->total_purchase - $contact->purchase_paid > 0)
                                                <a href="{{ action([\App\Http\Controllers\TransactionPaymentController::class, 'getPayContactDueCheck'], [$contact->id]) }}?type=purchase"
                                        ...
                                        @if ($contact->type == 'customer')
                                            <div class="clearfix"></div>
                                            <a href="{{ action([\App\Http\Controllers\TransactionPaymentController::class, 'getPayContactDueCheck'], [$contact->id]) }}?type=sell"
                                        ...
                                        <input type="hidden" id="selected_payment_id" name="selected_payment_id">
                                        {{-- end purchase button regle --}}
...
@section('javascript')
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            // start js regler
            $(document).on('click', 'a.payPurchase_due, a.paySale_due', function(e) {
                e.preventDefault(); // Prevent the default action of the link
                var selected_rows = getSelectedRows();
                var url = $(this).attr('href'); // Get the URL from the link

                if (selected_rows.length > 0) {
                    // Append selected row IDs to the URL as a query parameter
                    url += (url.includes('?') ? '&' : '?') + 'selected_row_ids=' + encodeURIComponent(JSON
                        .stringify(selected_rows));
