...
<div class="col-md-6 col-sm-6 col-xs-6 @if(!empty($for_pdf)) width-50 f-left @endif">
	<p class="blue-heading p-4 width-50">@lang('lang_v1.clientInfo'):</p>
	<p><strong>{{$contact->name}} {{$contact->supplier_business_name}}</strong>
...
<div class="col-md-6 col-sm-6 col-xs-6 text-right align-right @if(!empty($for_pdf)) width-100 f-left @endif">
	...
	<hr>
	<table class="table table-condensed text-left align-left no-border tdWitdh @if(!empty($for_pdf)) table-pdf @endif">
		<tr>
	...
	@if( $contact->type == 'supplier' || $contact->type == 'both')
		<tr style="color:green;">
			<td ><strong>@lang('report.total_purchase')</strong></td>
			<td class="align-right"><strong>@format_currency($ledger_details['total_purchase'])</strong></td>
		</tr>
	@endif
	@if( $contact->type == 'customer' || $contact->type == 'both')
		<tr style="color:green;">
			<td><strong>@lang('lang_v1.total_invoice')</strong></td>
			<td class="align-right"><strong>@format_currency($ledger_details['total_invoice'])</strong></td>
		</tr>
	@endif
	<tr>
	...
	<tr>
		<td>@lang('lang_v1.balance_due')</td>
		<td class="align-right">@format_currency($ledger_details['balance_due'] - $ledger_details['ledger_discount'])</td>
	</tr>
	<tr style="color:red;">
		<td><strong >@lang('lang_v1.balance_due_net')</strong></td>
		<td class="align-right"><strong>@format_currency($ledger_details['balance_due'] - $ledger_details['ledger_discount']-$contact->balance - $ledger_details['total_reverse_payment'])</strong></td>
	</tr>

	<tr style="color:orange;">
		<td><strong>@lang('lang_v1.uncashed_balance_due')</strong></td>
			<td class="align-right"><strong>
				@format_currency($ledger_details['balance_due1'])
		</strong></td>
	</tr>	
		...
		<tbody>
			@foreach($ledger_details['ledger'] as $data)
				<tr @if(!empty($for_pdf) && $loop->iteration % 2 == 0) class="odd" @endif>
					...
					@if($data['payment_method'] ==__('lang_v1.cheque'))
                    <td class="ws-nowrap align-right">{{ $data['cheque_type'] }}<br>{{ $data['cheque_number'] }}<br>({{ $data['cheque_status'] }})</td>
                    @else
                    <td class="ws-nowrap align-right">{{ $data['payment_method'] }}</td>
                    @endif
					<td>
						{!! $data['others'] !!}
