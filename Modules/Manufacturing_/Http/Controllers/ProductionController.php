...
class ProductionController extends Controller
{
    ...
    /////
    public function getRaportProduction(Request $request)
    {
        $business_id = request()->session()->get('user.business_id');
        if (!(auth()->user()->can('superadmin') || $this->moduleUtil->hasThePermissionInSubscription($business_id, 'manufacturing_module')) || !auth()->user()->can('manufacturing.access_production')) {
            abort(403, 'Unauthorized action.');
        ...
        if (request()->ajax()) {
            $productions = Transaction::join(
                ...
                ->where('transactions.business_id', $business_id)
                ->where('transactions.type', 'production_purchase')
                // ->whereIn('transactions.type', ['production_purchase', 'sell'])
                ->select(
            ...
            return Datatables::of($productions)
                ...
                ->editColumn('sale_quantity', function ($row) use ($request) {
                    $productionQuery = Transaction::join('business_locations AS bl', 'transactions.location_id', '=', 'bl.id')
                        ->join('transaction_sell_lines as pl', 'pl.transaction_id', '=', 'transactions.id')
                        ->leftJoin('units as su', 'pl.sub_unit_id', '=', 'su.id')
                        ->join('variations as v', 'v.id', '=', 'pl.variation_id')
                        ->join('product_variations as pv', 'pv.id', '=', 'v.product_variation_id')
                        ->join('products as p', 'p.id', '=', 'v.product_id')
                        ...
                        ->select(
                            DB::raw('sum(pl.quantity) as sale_quantity'),
                            'su.short_name as sub_unit_name',
                            'su.base_unit_multiplier',
                            'u.short_name as unit_name',
                        );
                    // Ajouter le filtrage supplémentaire en fonction des paramètres de requête

                    if ($request->has('location_id')) {
                        $locationId = $request->get('location_id');
                        if ($locationId) {
                            $productionQuery->where('transactions.location_id', $locationId);
                        }
                    }
                    ...
                    if ($request->has('start_date') && $request->has('end_date')) {
                        ...
                        if ($startDate && $endDate) {
                            $productionQuery->whereDate('transactions.transaction_date', '>=', $startDate)
                                ->whereDate('transactions.transaction_date', '<=', $endDate);
                        }
                    }
                    $productionQuery = $productionQuery->first();
                    $qty = empty($productionQuery->base_unit_multiplier) ? $productionQuery->sale_quantity : $productionQuery->sale_quantity / $productionQuery->base_unit_multiplier;
                    $unit = empty($productionQuery->sub_unit_name) ? $productionQuery->unit_name : $productionQuery->sub_unit_name;

                    return "<span class='display_currency' data-currency_symbol='false' data-orig-value='$qty' data-is_quantity='true'>$qty</span> $unit";
                })
                ->editColumn('sale_Details', function ($row) use ($request) {
                    $productionQuery = Transaction::join('business_locations AS bl', 'transactions.location_id', '=', 'bl.id')
                        ->join('transaction_sell_lines as pl', 'pl.transaction_id', '=', 'transactions.id')
                        ...
                        ->join('products as p', 'p.id', '=', 'v.product_id')
                        ->join('units as u', 'p.unit_id', '=', 'u.id')
                        ->where('v.product_id', $row->product_id)
                        ->where('transactions.type', 'sell');
                    ...
                    $productions = $productionQuery->select(
                        'transactions.id',
                        'c.name as contact',
                        'invoice_no',
                        'su.short_name as sub_unit_name',
                    ...
                    // Extraire les ref_nos de la collection
                    $contact = $productions->pluck('contact')->toArray();
                    $invoice_no = $productions->pluck('invoice_no')->toArray();
                    $qty = $productions->pluck('quantity')->toArray();
                    $unit = empty($productions->pluck('sub_unit_name')->toArray()) ?
                        $productions->pluck('sub_unit_name')->toArray() :
                        $productions->pluck('unit_name')->toArray();
                    $ids = $productions->pluck('id')->toArray();

                    $ref_nos_str = implode('<br>', array_map(function ($invoice_no, $contact, $qty, $unit, $id) {
                        if ($qty != 0) {
                            return '<a data-href="' . action([\App\Http\Controllers\SellController::class, 'show'], $id) . '" class="btn-modal" data-container=".view_modal">' .
                                $invoice_no . ' (<span style="color: green;">' . $this->productUtil->num_f($qty) . ' ' . $unit . '</span>)' . ($contact ? ' ' . $contact : '') . '</a>';
                        }
                    }, $invoice_no, $contact, $qty, $unit, $ids));

                    return $ref_nos_str;
                })
                ->editColumn('ref_no', function ($row) use ($request) {
                    $productionQuery = Transaction::join('business_locations AS bl', 'transactions.location_id', '=', 'bl.id')
                        ->join('purchase_lines as pl', 'pl.transaction_id', '=', 'transactions.id')
                        ->leftJoin('units as su', 'pl.sub_unit_id', '=', 'su.id')
                        ->join('variations as v', 'v.id', '=', 'pl.variation_id')
                    ...
                    // Extraire les ref_nos de la collection
                    $ref_nos = $productions->pluck('ref_no')->toArray();
                    $qty = $productions->pluck('quantity')->toArray();
                    $unit = empty($productions->pluck('sub_unit_name')->toArray()) ?
                        $productions->pluck('sub_unit_name')->toArray() :
                        $productions->pluck('unit_name')->toArray();
                    ...
                    $ref_nos_str = implode('<br>', array_map(function ($ref_no, $qty, $unit, $id) {
                        if ($qty != 0) {
                            return '<a data-href="' . action([\Modules\Manufacturing\Http\Controllers\ProductionController::class, 'show'], $id) . '" class="btn-modal" data-container=".view_modal">' . $ref_no . '(<span style="color: green;">' . $this->productUtil->num_f($qty) . $unit . '</span>' . ')</a>';
                        }
                    }, $ref_nos, $qty, $unit, $ids));

                    return $ref_nos_str;
                })
                ->editColumn('current_stock', function ($row) use ($request) {
                    $variation = $row->variation_id;
                    $location = $request->get('location_id');

                    $current_stock = \App\VariationLocationDetails::join('products as p', 'p.id', '=', 'variation_location_details.product_id')
                        ...
                        ->first();
                    return "<span class='display_currency' data-currency_symbol='false' data-orig-value='$current_stock->qty_available' data-is_quantity='true'>$current_stock->qty_available</span> $current_stock->unit ";
                })
                ->editColumn('total_quantity', function ($row) {
                    $qty = empty($row->base_unit_multiplier) ? $row->total_quantity : $row->total_quantity / $row->base_unit_multiplier;
                    $unit = empty($row->sub_unit_name) ? $row->unit_name : $row->sub_unit_name;

                    return "<span class='display_currency' data-currency_symbol='false' data-orig-value='$qty' data-is_quantity='true'>$qty</span> $unit";
                })
                ->editColumn('product_name', function ($row) {
                    $html = '<a href="' . action([\App\Http\Controllers\ProductController::class, 'productStockHistory'], [$row['product_id']]) . '">' . $row['product_name'] . '</a>';
                    return $html;
                })
                /*->editColumn('quantite_recente', function ($row) use ($request) {
                        /// Current Stock ---------------------
                            $variation = $row->variation_id;
                            $location = $request->get('location_id');
                            
                            $current_stock = \App\VariationLocationDetails::
                        ...
                        // Calculate quantite_recente
                        $quantite_recente = $current_stock->qty_available+$sale_qty-$total_quantity ;
                    
                        // Return the HTML for this column
                        if ($quantite_recente<0) {
                               $quantite_recente="--";
                              
                        }
                     
                       return "<span class='display_currency' data-currency_symbol='false' data-orig-value='$quantite_recente' data-is_quantity='true'>$quantite_recente</span> $unit";
                    })*/
                ->rawColumns(['final_total', 'sale_quantity', 'sale_Details', 'current_stock', 'ref_no', 'total_quantity', 'product_name'])
        ...
        $business_locations = BusinessLocation::forDropdown($business_id);
        return view('manufacturing::production.raport_production_fini', compact('business_locations'));
    }
    /////
    public function getRaportIngredient(Request $request)
    {
        $business_id = $request->session()->get('user.business_id');
        $business_locations = BusinessLocation::forDropdown($business_id);
        ...
        if (request()->ajax()) {
            $productions = Transaction::join('business_locations AS bl', 'transactions.location_id', '=', 'bl.id')
                ...
                ->whereIn('transactions.type', ['production_sell', 'production_purchase', 'sell'])
                ->select(
                    'p.category_id as category_id',
                    'v.product_id as product_id',
                    'v.id as variation_id',
                    'transactions.mfg_parent_production_purchase_id as production_purchase_id ',
                    'transactions.id',
                    'transaction_date',
                    'ref_no',
                    'bl.name as location_name',
            ...
            if ($request->has('category_id')) {
                ...
                if ($categoryId) {
                    $productions->whereHas('sell_lines.variations.product', function ($query) use ($categoryId) {
            ...
            return Datatables::of($productions)
                ...
                ->editColumn('sale_quantity', function ($row) use ($request) {
                    ...
                    if ($request->has('category_id')) {
                        $categoryId = $request->get('category_id');
                        if ($categoryId) {
                            $productionQuery->whereHas('sell_lines.variations.product', function ($query) use ($categoryId) {
                                $query->where('p.category_id', $categoryId);
                            });
                        }
                    }
                ...
                ->editColumn('sale_Details', function ($row) use ($request) {
                    ...
                    $ref_nos_str = implode('<br>', array_map(function ($invoice_no, $contact, $qty, $unit, $id) {
                        if ($qty != 0) {
                            return '<a data-href="' . action([\App\Http\Controllers\SellController::class, 'show'], $id) . '" class="btn-modal" data-container=".view_modal">' .
                                $invoice_no . ' (<span style="color: red;">' . $this->productUtil->num_f($qty) . ' ' . $unit . '</span>)' . ($contact ? ' ' . $contact : '') . '</a>';
                        }
                    }, $invoice_no, $contact, $qty, $unit, $ids));
                ...
                ->editColumn('ref_no', function ($row) use ($request) {
                    ...
                    // Ajoutez le filtrage supplémentaire en fonction des paramètres de requête
                    if ($request->has('category_id')) {
                        $categoryId = $request->get('category_id');
                        if ($categoryId) {
                            $productionQuery->whereHas('sell_lines.variations.product', function ($query) use ($categoryId) {
                    ...
                    $production = $productionQuery->select(
                        'transactions.mfg_parent_production_purchase_id as production_purchase_id',
                        'su.short_name as sub_unit_name',
                        'pl.quantity as quantity',
                        'u.short_name as unit_name'
                    ...
                    if ($production->isNotEmpty()) {
                        $id_parent = $production->pluck('production_purchase_id')->toArray();
                        $qty = $production->pluck('quantity')->toArray();
                        $unit = empty($production->pluck('sub_unit_name')->toArray()) ?
                            $production->pluck('sub_unit_name')->toArray() :
                            $production->pluck('unit_name')->toArray();

                        $production_purchase = Transaction::whereIn('id', $id_parent)->get();

                        $production_purchase2 = Transaction::join(
                            'business_locations AS bl',
                            'transactions.location_id',
                            '=',
                            'bl.id'
                            ...
                            ->join('product_variations as pv', 'pv.id', '=', 'v.product_variation_id')
                            ->join('products as p', 'p.id', '=', 'v.product_id')
                            ->whereIn('transactions.id', $id_parent)
                            ->select(
                        ...
                        // Extraction des ref_nos, quantités, unités et IDs
                        $ref_nos = $production_purchase->pluck('ref_no')->toArray();
                        $ids = $production_purchase->pluck('id')->toArray();
                        $name = $production_purchase2->pluck('product_name')->toArray();
                        $ref_nos_str = implode('<br>', array_map(function ($ref_no, $qty, $unit, $id, $name) {
                            if ($qty != 0) {
                                return '<a data-href="' . action([\Modules\Manufacturing\Http\Controllers\ProductionController::class, 'show'], $id) . '" class="btn-modal" data-container=".view_modal">' . $ref_no . '(<span style="color: red;">' . $this->productUtil->num_f($qty) . $unit . '</span>) ' . $name . '</a>';
                            }
                        }, $ref_nos, $qty, $unit, $ids, $name));
                    }
                ...
                ->editColumn('product_name', function ($row) {
                    ...
                    $is_red = $categoryId == 2 ? 'style="color: red;"' : '';

                    $html = '<a href="' . action([\App\Http\Controllers\ProductController::class, 'productStockHistory'], [$row->product_id]) . '" ' . $is_red . '>' . $row->product_name . '</a>';
                    return $html;
                })
                ...
                /* ->editColumn('quantite_recente', function ($row) use ($request) {
                       ...
                       // return "<span>$current_stock->qty_available</span>";
                    })*/
                ->rawColumns(['final_total', 'sale_quantity', 'sale_Details', 'current_stock', 'ref_no', 'total_quantity', 'product_name'])
        ...
        return view('manufacturing::production.raport_Ingredient')->with(compact('business_locations', 'categories'));
    }
