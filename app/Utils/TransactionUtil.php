...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            //
            'facture_numero' =>!empty($input['facture_numero']) ? str_pad( $input['facture_numero'], 4, '0', STR_PAD_LEFT) :null,
            'facture_first' => !empty($input['facture_numero']) ? true : null,
            'facture_date' => !empty($input['facture_numero']) ? \Carbon::now() : null,
            //            
    ...
    public function updateSellTransaction($transaction_id, $business_id, $input, $invoice_total, $user_id, $uf_data = true, $change_invoice_number = true)
    {
        ...
        $transactionPayment->payment_for = $input['contact_id'];
            ...
            if (!empty($payment['payment_id'])) {
                ...
                if ($payment_amount != 0) {
                    ...
                    $payment_data = [
                        ...
                        'cheque_echeance' => isset($payment['cheque_echeance']) ? $payment['cheque_echeance'] : null,
                        'cheque_status' => isset($payment['cheque_status']) ? $payment['cheque_status'] : null,
                        'bank_name' => isset($payment['bank_name']) ? $payment['bank_name'] : null,
                        'cheque_type' => isset($payment['cheque_type']) ? $payment['cheque_type'] : null,
                        'cheque_owner' => isset($payment['cheque_owner']) ? $payment['cheque_owner'] : null,
    ...
    public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type)
    {
        ...
        if ($il->show_customer == 1) {
            ...
            if ($receipt_printer_type != 'printer') {
                $output['customer_info'] .= '<b>' . $output['customer_name'] . '</b>';
        ...
        $output['facture_numero'] = $transaction->facture_numero;
            ...
            foreach ($due_transactions as $transaction) {
                ...
                if ($total_amount > 0) {
                    ...
                    $array = [
                        ...
                        'cheque_owner'=>$parent_payment->cheque_owner,
                        ...
                        'cheque_echeance' => $parent_payment->cheque_echeance,
    ...
    // Insertion advenace Transaction 
    public function createAdvanceTransaction($business_id, $contact_id, $amount, $created_by, $uf_data = true)
    {
        $business_location = BusinessLocation::where('business_id', $business_id)
            ->first();
        $final_amount = $uf_data ? $this->num_uf($amount) : $amount;
    ...
    public function getListPurchases($business_id)
    {
        $purchases = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ...
            ->where('transactions.business_id', $business_id)
            ->where('transactions.type', 'purchase')
            //->whereNull('transactions.facture_first')
            ->select(
                'transactions.facture_numero as facture_numero',
                'transactions.id',
    ...
    public function getListPurchasesFacture($business_id)
    {
        $purchases = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ->join(
                'business_locations AS BS',
            ...
            ->where('transactions.business_id', $business_id)
            ->where('transactions.type', 'purchase')
            ->whereNotNull('transactions.facture_numero')
            ->select(
                'transactions.facture_date as facture_date',
                'transactions.facture_numero as facture_numero',
                'transactions.id',
                'transactions.document',
                'transactions.transaction_date',
    ...
    public function getListSellsFacture($business_id, $sale_type = 'sell')
    {
        $sells = Transaction::leftJoin('contacts', 'transactions.contact_id', '=', 'contacts.id')
            ->leftJoin('transaction_payments as tp', 'transactions.id', '=', 'tp.transaction_id')
            ->leftJoin('transaction_sell_lines as tsl', function ($join) {
    ...
    public function getLedgerDetails($contact_id, $start, $end, $format = 'format_1', $location_id = null, $line_details = false)
    {
        ...
        foreach ($transactions as $transaction) {
            ...
            $temp_array = [
                ...
                'total' => '',
                'payment_method' => '',
                // 'cheque_type' => $transaction->cheque_type,
                'cheque_number' => $transaction->cheque_number,
                'cheque_status' => $transaction->cheque_status,
        ...
        // $balance_due_no_encaisse = $transactions->where('tp.cheque_status', '!=', 'encaissé')->sum('final_total');
        ...
        $total_advance_payment = $this->__paymentQuery($contact_id, $start, $end, $location_id)
            ...
            ->sum('amount');
        ///
        $balance_due = $this->__paymentQuery($contact_id, $start, $end, $location_id)
            ->where('transaction_payments.method', 'cheque')
            ->where('transaction_payments.cheque_status', '!=' , 'Encaissé')
    ...
    public function updateContactTransferd($id)
    {
        $business_id = auth()->user()->business_id;
        $transaction = TransactionPayment::where('id', $id)
            ->where('business_id', $business_id)
            ->first();
    
        if ($transaction) {
            $transaction->cheque_transfered_id = 1;
            $transaction->save();
        }
    }
    ...
    public function payContact($request, $format_data = true)
    {
        ...
        $business_id = auth()->user()->business_id;
        $cheque_transfered=$request->input('transaction_id');
        ...
        $inputs['cheque_transfered_id'] = $cheque_transfered;
        ...
        if($request['is_advance']=='is_advance'){
            $advance_transaction=$this->createAdvanceTransaction($business_id, $contact->id, $inputs['amount'], $inputs['created_by'], false);
            $inputs['transaction_id']=$advance_transaction->id;
        }
        ...
        if (!empty($cheque_transfered)) {
            $this->updateContactTransferd($cheque_transfered);
        }
