        ...
        if (request()->ajax()) {
            ...
            ->editColumn('amount', function ($row) {
                ...
                ->editColumn('action', function ($row) {
                        ...
                        $query1 = null;
                        $error = '';
                        if ($row->cheque_transfered_id) {
                            $query1 = TransactionPayment::find($row->cheque_transfered_id);
                        
                            if ($query1 && $query1->cheque_status != $row->chequeStatus) {
                                $error = '<button type="button" id="edit_payment" class="btn btn-warning btn-xs edit_payment" data-href="' . action([\App\Http\Controllers\TransactionPaymentController::class, 'edit3'], [$row->cheque_transfered_id]) . '">
                        ...
                        if ($row->id_invoice == null) {
                            return  $viewPay.' '.$error;
                        } else {
                            return $editButton.' '. $viewPay.' '.$error;
                        }
                    })                      
                ->removeColumn('id')
