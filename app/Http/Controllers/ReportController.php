...
class ReportController extends Controller
{
            ...
            return Datatables::of($contacts)
                ...
                ->addColumn('due', function ($row) {
                    ...
                    if ($row->contact_type == 'supplier') {
                        $due -= $row->opening_balance - $row->opening_balance_paid-$row->balance;
                    } else {
                        $due += $row->opening_balance - $row->opening_balance_paid-$row->balance;
                    }
                    ...
                    if ($row->contact_type == 'supplier') {
                        return '<span class="total_due" style="color: '.($due < 0 ? 'red' : 'green').'" data-orig-value="'.$due.'">'.$due_formatted.'</span>';
                    } else {
                        return '<span class="total_due" style="color: '.($due < 0 ? 'green' : 'red').'" data-orig-value="'.$due.'">'.$due_formatted.'</span>';
                    }
    ...
    public function getStockReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            $filters = request()->only(['location_id', 'category_id', 'sub_category_id','sub_sub_category_id', 'brand_id', 'unit_id', 'tax_id', 'type',
            ...
            return Datatables::of($registers)
                ...
                ->addColumn('action', '<button type="button" data-href="{{action(\'App\Http\Controllers\CashRegisterController@show\', [$id])}}" class="btn btn-xs btn-info btn-modal" 
                    data-container=".view_register"><i class="fas fa-eye" aria-hidden="true"></i> @lang("messages.view")</button>
                     @if($status != "close" && auth()->user()->can("close_cash_register"))<button type="button" data-href="{{action(\'App\Http\Controllers\CashRegisterController@getCloseRegister\', [$id])}}" class="btn btn-xs btn-danger btn-modal" 
        ...
        if ($request->ajax()) {
            ...
            if (! empty($purchase_line)) {
                if (! empty($purchase_line->exp_date)) {
                    ...
                    $this->transactionUtil->format_date($purchase_line->exp_date,false);
                }
            }
    ...
    public function purchasePaymentReport(Request $request)
    {
        ...
        $business_id = $request->session()->get('user.business_id');
        $prefix_purchase_payement = request()->session()->get('business.ref_no_prefixes')['purchase_payment'];
        if ($request->ajax()) {
            $supplier_id = $request->get('supplier_id', null);
            ...
            $query = TransactionPayment::leftjoin('transactions as t', function ($join) use ($business_id) {
                $join->on('transaction_payments.transaction_id', '=', 't.id')
                    ->where('t.business_id', $business_id)
                    ->whereIn('t.type', ['purchase', 'opening_balance','advance']);
            })
                ->where('transaction_payments.business_id', $business_id)
                ->where(function ($q) use ($business_id, $contact_filter1, $contact_filter2, $parent_payment_query_part) {
                    $q->whereRaw("(transaction_payments.transaction_id IS NOT NULL AND t.type IN ('purchase', 'opening_balance','advance')  $parent_payment_query_part $contact_filter1)")
                        ->orWhereRaw("EXISTS(SELECT * FROM transaction_payments as tp JOIN transactions ON tp.transaction_id = transactions.id WHERE transactions.type IN ('purchase', 'opening_balance','advance') AND transactions.business_id = $business_id AND tp.parent_id=transaction_payments.id $contact_filter2)");
                })
                ->where('transaction_payments.method', '!=', 'advance')
                ->where('transaction_payments.payment_ref_no', 'LIKE',$prefix_purchase_payement.'%' )
                ->select(
                    ...
                    'transaction_payments.cheque_type',
            ...
            return Datatables::of($query)
                ...
                ->editColumn('method', function ($row) use ($payment_types) {
                    if ($row->method == 'cheque') {
                        $method = ! empty($row->cheque_type) ? $row->cheque_type : '';
                    }
                    else{
    ...
    public function sellPaymentReport(Request $request)
    {
        ...
        $business_id = $request->session()->get('user.business_id');
        $prefix_sell_payement = request()->session()->get('business.ref_no_prefixes')['sell_payment'];
        ...
        if ($request->ajax()) {
            ...
            $query = TransactionPayment::leftjoin('transactions as t', function ($join) use ($business_id) {
                $join->on('transaction_payments.transaction_id', '=', 't.id')
                    ->where('t.business_id', $business_id)
                    ->whereIn('t.type', ['sell', 'opening_balance','advance']);
            })
                ->leftjoin('contacts as c', 't.contact_id', '=', 'c.id')
                ->leftjoin('customer_groups AS CG', 'c.customer_group_id', '=', 'CG.id')
                ->where('transaction_payments.business_id', $business_id)
                ->where(function ($q) use ($business_id, $contact_filter1, $contact_filter2, $parent_payment_query_part) {
                    $q->whereRaw("(transaction_payments.transaction_id IS NOT NULL AND t.type IN ('sell', 'opening_balance','advance') $parent_payment_query_part $contact_filter1)")
                ...
                ->where('transaction_payments.method', '!=', 'advance')
                ->where('transaction_payments.payment_ref_no', 'LIKE',$prefix_sell_payement.'%' )
                ->select(
                    DB::raw("IF(transaction_payments.transaction_id IS NULL, 
                                (SELECT c.name FROM transactions as ts
                                JOIN contacts as c ON ts.contact_id=c.id 
            ...
            if (! empty($request->get('cheque_type'))) {
                $query->where('transaction_payments.cheque_type', $request->get('cheque_type'));
    ...
    public function getPaymentReport(Request $request)
    {
        ...
        return view('report.payment_report', compact(
            'business_locations',
            'users'
        ));
    }
...
public function postPaymentReport(Request $request)
{
    ...
    $users_id = $request->register_user_id;

    $transactionTypes = ['expense', 'sell_return', 'purchase_return'];
    $transactionData = $this->getTransactionData($transactionTypes, $users_id, $location_id, $date_start, $date_end);
    
    $register_details = $this->getRegisterDetails($users_id, $location_id);

    $transaction_details = $this->getTransactionDetails('sell', $users_id, $location_id, $date_start, $date_end);
    $details = $this->getProductsSellDetails('sell', $users_id, $location_id, $date_start, $date_end);

    $transaction_details_purchase = $this->getTransactionDetails('purchase', $users_id, $location_id, $date_start, $date_end);
    $details_purchase = $this->getProductPurchaseDetails('purchase',$users_id, $location_id, $date_start, $date_end);
    $SellTransaction = $this->getSellTransaction($date_start, $date_end, $location_id, $users_id);
    $PurchaseTransaction=$this->getPurchaseTransaction($date_start, $date_end, $location_id, $users_id);
...
private function getRegisterDetails($users_id, $location_id)
{
    ...
    if (!empty($users_id)) {
        $register_details->where('cash_registers.user_id', $users_id);
    }
    if (!empty($location_id)) {
        $register_details->where('cash_registers.location_id', $location_id);
    }

    return $register_details->first();
}

private function getTransactionDetails($type, $users_id, $location_id, $date_start, $date_end)
{
    $transaction_details = Transaction::where('transactions.type', $type)
    ->where('transactions.status', 'final')
    ->select(
    ...
    if (!empty($users_id)) {
        $transaction_details->where('transactions.created_by', $users_id);
    }
    if (!empty($location_id)) {
        $transaction_details->where('transactions.location_id', $location_id);
    }
    if (!empty($date_start) && !empty($date_end)) {
        $transaction_details->whereBetween('transactions.transaction_date', [$date_start, $date_end]);
    } else {
        if (!empty($date_start)) {
            $transaction_details->whereDate('transactions.transaction_date', '>=', $date_start);
        }
    
        if (!empty($date_end)) {
            $details->whereDate('transactions.transaction_date', '<=', $date_end);
        }
    }

    return $transaction_details->first();
}

private function getProductsSellDetails($type, $users_id, $location_id, $date_start, $date_end)
{
    $details = Transaction::where('transactions.type', $type)
    ->where('transactions.status', 'final')
    ...
    if (!empty($users_id)) {
        $details->where('transactions.created_by', $users_id);
    }
    if (!empty($location_id)) {
        $details->where('transactions.location_id', $location_id);
    }
    if (!empty($users_id)) {
        $details->where('transactions.created_by', $users_id);
    }
    ...
    if (!empty($date_start) && !empty($date_end)) {
        $details->whereBetween('transactions.transaction_date', [$date_start, $date_end]);
    } else {
        if (!empty($date_start)) {
            $details->whereDate('transactions.transaction_date', '>=', $date_start);
        }
...
private function getProductPurchaseDetails($type, $users_id, $location_id, $date_start, $date_end)
{
    $productPurchaseDetails = Transaction::where('transactions.type', $type)
    ...
    if (!empty($users_id)) {
        $productPurchaseDetails->where('transactions.created_by', $users_id);
    }

    if (!empty($location_id)) {
        $productPurchaseDetails->where('transactions.location_id', $location_id);
    }

    if (!empty($date_start) && !empty($date_end)) {
        $productPurchaseDetails->whereBetween('transactions.transaction_date', [$date_start, $date_end]);
    } else {
        if (!empty($date_start)) {
            $productPurchaseDetails->whereDate('transactions.transaction_date', '>=', $date_start);
        }

        if (!empty($date_end)) {
            $productPurchaseDetails->whereDate('transactions.transaction_date', '<=', $date_end);
        }
    }
    $detailsPurchase = $productPurchaseDetails->get();

    return $detailsPurchase;
}

private function getTransactionData($transactionTypes, $users_id, $location_id, $date_start)
{
    $transactionData = [];

    foreach ($transactionTypes as $type) {
        $transactionData[$type] = Transaction::leftJoin('transaction_payments as t', function ($join) use ($type) {
            $join->on('transactions.id', '=', 't.transaction_id')
                ->where('transactions.type', $type)
                ->whereNull('t.parent_id');
        })
            ->select(
                DB::raw("SUM(CASE WHEN t.method='cheque' THEN t.amount ELSE 0 END) as amount_cheque"),
                DB::raw("SUM(CASE WHEN t.method='cash' THEN t.amount ELSE 0 END) as amount_cash"),
                DB::raw("SUM(CASE WHEN t.method='card' THEN t.amount ELSE 0 END) as amount_card"),
                DB::raw("SUM(CASE WHEN t.method='bank_transfer' THEN t.amount ELSE 0 END) as amount_bank_transfer")
            );

        if (!empty($users_id)) {
            $transactionData[$type]->where('t.created_by', $users_id);
        }

        if (!empty($location_id)) {
            $transactionData[$type]->where('transactions.location_id', $location_id);
        }

        if (!empty($date_start) && !empty($date_end)) {
            $transactionData[$type]->whereBetween('t.paid_on', [$date_start, $date_end]);
        }
        if (!empty($date_start)) {
            $transactionData[$type]->whereDate('t.paid_on', '>=', $date_start);
        }        

        $transactionData[$type] = $transactionData[$type]->first();
    }

    return $transactionData;
}

private function getSellTransaction($date_start, $date_end, $location_id, $users_id) 
{
    $business_id = auth()->user()->business_id;
    $prefix_sell_payement = request()->session()->get('business.ref_no_prefixes')['sell_payment'];
    $parent_payment_query_part = empty($location_id) ? 'AND transaction_payments.parent_id IS NULL' : '';
    $query = TransactionPayment::leftJoin('transactions as t', function ($join) use ($business_id) {
        ...
        ->where(function ($q) use ($business_id, $parent_payment_query_part) {
            $q->whereRaw("(transaction_payments.transaction_id IS NOT NULL AND t.type IN ('sell', 'opening_balance','advance') $parent_payment_query_part )")
                ->orWhereRaw("EXISTS(SELECT * FROM transaction_payments as tp JOIN transactions ON tp.transaction_id = transactions.id WHERE transactions.type IN ('sell', 'opening_balance') AND transactions.business_id = $business_id AND tp.parent_id=transaction_payments.id)");
        })
        ->where('transaction_payments.method', '!=', 'advance')
        ->where('transaction_payments.payment_ref_no', 'LIKE',$prefix_sell_payement.'%' )
        ->select(
            DB::raw("SUM(CASE WHEN transaction_payments.method='cheque' THEN transaction_payments.amount ELSE 0 END) as amount_cheque"),
            DB::raw("SUM(CASE WHEN transaction_payments.method='cash' THEN transaction_payments.amount ELSE 0 END) as amount_cash"),
            DB::raw("SUM(CASE WHEN transaction_payments.method='card' THEN transaction_payments.amount ELSE 0 END) as amount_card"),
            DB::raw("SUM(CASE WHEN transaction_payments.method='bank_transfer' THEN transaction_payments.amount ELSE 0 END) as amount_bank_transfer")
        );

    if (!empty($date_start) && !empty($date_end)) {
        $query->whereBetween(DB::raw('date(transaction_payments.paid_on)'), [$date_start, $date_end]);
        
    }
    if (!empty($date_start) ) {
       $query->whereDate(DB::raw('date(transaction_payments.paid_on)'), $date_start);
    ...
    if (!empty($users_id)) {
        $query->where('transaction_payments.created_by', $users_id);
    }

    $transactionSell = $query->first();
    return $transactionSell;
}

private function getPurchaseTransaction($date_start, $date_end, $location_id, $users_id) 
{
    $business_id = auth()->user()->business_id;
    $prefix_purchase_payement = request()->session()->get('business.ref_no_prefixes')['purchase_payment'];
    $parent_payment_query_part = empty($location_id) ? 'AND transaction_payments.parent_id IS NULL' : '';
    ...
    $query = TransactionPayment::leftjoin('transactions as t', function ($join) use ($business_id) {
        ...
        ->where('transaction_payments.business_id', $business_id)
        ->where(function ($q) use ($business_id , $parent_payment_query_part) {
            $q->whereRaw("(transaction_payments.transaction_id IS NOT NULL AND t.type IN ('purchase', 'opening_balance','advance')  $parent_payment_query_part)")
                ->orWhereRaw("EXISTS(SELECT * FROM transaction_payments as tp JOIN transactions ON tp.transaction_id = transactions.id WHERE transactions.type IN ('purchase', 'opening_balance') AND transactions.business_id = $business_id AND tp.parent_id=transaction_payments.id )");
        })
        ->where('transaction_payments.method', '!=', 'advance')
        ->where('transaction_payments.payment_ref_no', 'LIKE',$prefix_purchase_payement.'%' )
        ->select(
    ...
    $transactionPurchase = $query->first();
