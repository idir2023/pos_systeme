    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
                ...
                $menu->dropdown(
                    ...
                    function ($sub) {
                        if (auth()->user()->can('customer.view') || auth()->user()->can('customer.view_own')) {
                            ...
                            $sub->url(
                                action([\App\Http\Controllers\ContactController::class, 'show'], 1),
                                __('contact.customer_situation'),
                                ['icon' => 'fa fas fa-star', 'active' => request()->segment(1) == 'contacts'&& request()->segment(2) == '1']
                            );
                            $sub->url(
                                action([\App\Http\Controllers\CustomerGroupController::class, 'index']),
                                __('lang_v1.customer_groups'),
                                ['icon' => 'fa fas fa-users',
                                'class' => (session('business.customer_groups') ? '' : 'hide'),
                        ...
                        if (auth()->user()->can('supplier.create') || auth()->user()->can('customer.create')) {
                            $sub->url(
                                ...
                                'class' => (session('business.import_contacts') ? '' : 'hide'),
                    ...
                    function ($sub) {
                        if (auth()->user()->can('supplier.view') || auth()->user()->can('supplier.view_own')) {
                            $sub->url(
                              ...
                              $sub->url(
                                action([\App\Http\Controllers\ContactController::class, 'show'], 2),
                                __('contact.supplier_situation'),
                                ['icon' => 'fa fas fa-star', 'active' => request()->segment(1) == 'contacts'&& request()->segment(2) == '2']
                            );
                        }
                        if (auth()->user()->can('supplier.create') || auth()->user()->can('customer.create')) {
                            $sub->url(
                    ...
                    function ($sub) {
                        ...
                        if (auth()->user()->can('product.create')) {
                            $sub->url(
                                ...
                                'class' => (session('business.update_product_price') ? '' : 'hide'),
                                'active' => request()->segment(1) == 'update-product-price']
                            );
                        }
                        if (auth()->user()->can('product.view')) {
                            $sub->url(
                                ...
                                'class' => (session('business.print_labels') ? '' : 'hide'),
                        ...
                        if (auth()->user()->can('product.create')) {
                            ...
                            $sub->url(
                                ...
                                ['icon' => 'fa fas fa-download',
                                'class' => (session('business.enable_import') ? '' : 'hide'),
                        ...
                        if (auth()->user()->can('product.opening_stock')) {
                            $sub->url(
                                ...
                                ['icon' => 'fa fas fa-download',
                                'class' => (session('business.enable_import_stock') ? '' : 'hide'),
                        ...
                        $sub->url(
                            ...
                            'class' => (session('business.warranties') ? '' : 'hide'),
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($common_settings) {
                        ...
                        if (auth()->user()->can('purchase.view')) {
                            $sub->url(
                                action([\App\Http\Controllers\PurchaseController::class, 'getFacture']),
                                __('messages.facture'),
                                ['icon' => 'fa fas fa-list',
                                'class' => (session('business.facture') ? '' : 'hide'),
                                'active' => request()->segment(1) == 'purchases' && request()->segment(2) == 'facture']
                            );
                            }
                        //
                    ...
                    function ($sub) use ($enabled_modules, $is_admin, $pos_settings) {
                         if (in_array('add_sale', $enabled_modules) && ($is_admin || auth()->user()->hasAnyPermission(['quotation.view_all', 'quotation.view_own']))) {
                            $sub->url(
                                ...
                                ['icon' => 'fa fas fa-pen-square',
                                'class' => (session('business.list_quotations') ? '' : 'hide'),
                                'active' => request()->segment(1) == 'sells' && request()->segment(2) == 'quotations']
                            );
                        }
                        ...
                        if ($is_admin || auth()->user()->hasAnyPermission(['sell.view', 'sell.create', 'direct_sell.access', 'direct_sell.view', 'view_own_sell_only', 'view_commission_agent_sell', 'access_shipping', 'access_own_shipping', 'access_commission_agent_shipping'])) {
                        $sub->url(
                            action([\App\Http\Controllers\SellController::class, 'getFacture']),
                            __('messages.facture'),
                            ['icon' => 'fa fas fa-list',
                            'class' => (session('business.facture') ? '' : 'hide'),
                            'active' => request()->segment(1) == 'sells' && request()->segment(2) == 'facture']
                        );
                        }
                        /////
                        ...
                        if (auth()->user()->can('access_sell_return') || auth()->user()->can('access_own_sell_return')) {
                            $sub->url(
                                action([\App\Http\Controllers\SellReturnController::class, 'getSellReturn']),
                                __('Retour des Ventes'),
                                ['icon' => 'fa fas fa-undo', 'active' => request()->segment(1) == 'get-sell-return' && request()->segment(2) ==null]
                            );
                        }
                        /*
                        if (in_array('add_sale', $enabled_modules) && auth()->user()->can('direct_sell.access')) {
                            $sub->url(
                                ...
                                'class' => (session('business.add_quotation') ? '' : 'hide'),
                                'active' => request()->get('status') == 'quotation']
                            );
                        }
                        ...
                        if (in_array('add_sale', $enabled_modules) && auth()->user()->can('direct_sell.access')) {
                            $sub->url(
                                ...
                                ['icon' => 'fa fas fa-plus-circle',
                                'class' => (session('business.add_draft') ? '' : 'hide'),
                        ...
                        if (in_array('add_sale', $enabled_modules) && ($is_admin || auth()->user()->hasAnyPermission(['draft.view_all', 'draft.view_own']))) {
                            $sub->url(
                                ...
                                ['icon' => 'fa fas fa-pen-square', 
                                'class' => (session('business.list_drafts') ? '' : 'hide'),
                                'active' => request()->segment(1) == 'sells' && request()->segment(2) == 'drafts']
                            );
                        }
                        ...
                        if ($is_admin || auth()->user()->hasAnyPermission(['access_shipping', 'access_own_shipping', 'access_commission_agent_shipping'])) {
                            $sub->url(
                                ...
                                'class' => (session('business.shipments') ? '' : 'hide'),
                        ...
                        if (auth()->user()->can('discount.access')) {
                            $sub->url(
                                ...
                                'class' => (session('business.discounts') ? '' : 'hide'),
                        ...
                        if (auth()->user()->can('sell.create')) {
                            $sub->url(
                                ...
                                'class' => (session('business.import_sales') ? '' : 'hide'),
            ...
            if (in_array('expenses', $enabled_modules) && (auth()->user()->can('all_expense.access') || auth()->user()->can('view_own_expense'))) {
                ...
                if (auth()->user()->can('delete_sell_payment')) {
                    $menu->dropdown(
                        __('messages.cheque'),
                        function ($sub) {
                            if (auth()->user()->can('expense.add')) {
                                ...
                                $sub->url(
                                    action([\App\Http\Controllers\ChequeClientController::class, 'index']),
                                    __('messages.cheque_customer'),
                                    ['icon' => 'fa fa-file-invoice-dollar', 'active' => request()->segment(1) == '']
                                );
                                $sub->url(
                                    action([\App\Http\Controllers\ChequeFournisseurController::class, 'index']),
                                    __('messages.cheque_suppliers'),
                                    ['icon' => 'fa fa-file-invoice-dollar', 'active' => request()->segment(1) == '']
                                );
                                $sub->url(
                                    action([\App\Http\Controllers\ChequeDepenseController::class, 'index']),
                                    __('messages.cheque_expenses'),
                                    ['icon' => 'fa fa-file-invoice-dollar', 'active' => request()->segment(1) == '']
                                );

                                $sub->url(
                                    action([\App\Http\Controllers\ChqsClientController::class, 'index']),
                                    __('messages.cheque_customer_garantie'),
                                    ['icon' => 'fa fas fa-money-check', 'active' => request()->segment(1) == 'Client']
                                );
                                $sub->url(
                                    action([\App\Http\Controllers\ChqsFournisseurController::class, 'index']),
                                    __('messages.cheque_supplier_garantie'),
                                    ['icon' => 'fa fa-file-invoice-dollar', 'active' => request()->segment(1) == 'Fournisseur']
                                );
                            }
                        },
                ...
                $menu->dropdown(
                    ...
                    function ($sub) use ($enabled_modules, $is_admin) {
                        ...
                        if (auth()->user()->can('payment_report.view')) {
                            $sub->url(
                                action([\App\Http\Controllers\ReportController::class, 'getPaymentReport']),
                                __('report.payment_report'),
                                ['icon' => 'fa fas fa-briefcase', 'active' => request()->segment(2) == 'payment-report']
                            );
                        }
                        // 
            ...
            if (auth()->user()->can('business_settings.access') ||
                ...
                $menu->dropdown(
                    ...
                    //Cheque User management dropdown
