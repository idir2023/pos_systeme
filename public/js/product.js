...
$(document).ready(function () {
    // Check if category dropdown has a value
    var categoryId = $('select[name="category_id"]').val();
    var subCategoryDropdown = $('#sub_category');
    
    if (!categoryId) {
        subCategoryDropdown.addClass('hide');
    } else {
        subCategoryDropdown.removeClass('hide');
    }
   
    var categoryId1 = $('select[name="sub_category_id"]').val();
    if (categoryId1 === '') {
        subCategoryDropdown.addClass('hide');
    } else {
        subCategoryDropdown.removeClass('hide');
    ...
    // Check if sub-category dropdown has a value
    var subCategoryId = $('select[name="sub_category_id"]').val();
    var subSubCategoryDropdown = $('#sub_sub_category');
    if (!subCategoryId) {
        subSubCategoryDropdown.addClass('hide');
    } else {
        subSubCategoryDropdown.removeClass('hide');
    }

    // Check if sub-sub-category dropdown has the value 'None'
    var subCategoryId1 = $('select[name="sub_sub_category_id"]').val();
    if (subCategoryId1 === '') {
        subSubCategoryDropdown.addClass('hide');
    } else {
        subSubCategoryDropdown.removeClass('hide');
    ...
    $(document).on('change', 'select[name="category_id"]', function () {
        ...
        if (categoryId) {
            // Fetch sub-categories using AJAX
            $.ajax({
                method: 'POST',
                ...
                success: function (result) {
                    if (result) {
                        ...
                        // Show or hide sub-category dropdown based on result
                        if (result.trim() == '<option value="">None</option>') {
                            subCategoryDropdown.addClass('hide');
                        } else {
                            subCategoryDropdown.removeClass('hide');
                        }
    ...
    // Event listener for sub-category dropdown change
    $(document).on('change', 'select[name="sub_category_id"]', function () {
        var subCategoryId = $(this).val();
        var subSubCategoryDropdown = $('#sub_sub_category');
        var subSubCategorySelect = $('select[name="sub_sub_category_id"]');

        if (subCategoryId) {
            // Fetch sub-sub-categories using AJAX
            $.ajax({
                method: 'POST',
                url: '/products/get_sub_sub_categories',
                dataType: 'html',
                data: { sub_cat_id: subCategoryId },
                success: function (result) {
                    if (result) {
                        // Replace the existing options with the new ones
                        subSubCategorySelect.html(result);
                        // Show or hide sub-sub-category dropdown based on result
                        if (result.trim() == '<option value="">None</option>') {
                            subSubCategoryDropdown.addClass('hide');
                        } else {
                            subSubCategoryDropdown.removeClass('hide');
