$(document).ready(function() {
    ...
    var factureNumeroContainer = $('#facture_numero_container');
    var factureNumeroInput = $('#facture_numero');
    var invoice_no = $('.invoice_no');


    $('#only_facture').on('ifChanged', function(event){
        if ($(this).is(':checked')) {
            factureNumeroContainer.removeClass('hide'); 
            factureNumeroInput.val(factureNumeroInput.data('facture_numero')).trigger('change');
            //invoice_no.val('---').trigger('change');

        } else {
            factureNumeroContainer.addClass('hide');
            factureNumeroInput.val('').trigger('change');
            invoice_no.val('').trigger('change');
        }
    });
    if (factureNumeroContainer.hasClass('hide')) {
        factureNumeroInput.val('').trigger('change');
    }
...
function calculate_balance_due() {
    ...
    $('.cancel-payment-row').on('click', function(event) {
        $('#change_payment_data').find('.input_number1').val('').change();
        $('#change_payment_data').addClass('hide'); // Hide the modal*
    ...
    if (bal_due !== 0) {
        ...
        $('#change_payment_data').removeClass('hide');
        var advanceAmount = parseFloat($('#advance_balance').val()) || 0;
        __write_number($('input#in_balance_due'), bal_due);
        $('span.balance_due').text(__currency_trans_from_en(bal_due, true));

        if (advanceAmount > 0) {
            if (advanceAmount < bal_due) {
                $(".input_number1").val(advanceAmount);
                __write_number($('input#in_balance_due'),bal_due - advanceAmount);
                $('span.balance_due').text(__currency_trans_from_en(bal_due-advanceAmount, true));
            } else {
                $(".input_number1").val(bal_due);
                __write_number($('input#in_balance_due'),0);
                $('span.balance_due').text(__currency_trans_from_en(0, true));
            }
        } else {
            $('#change_payment_data').addClass('hide');
        }
    } else {
        $('#change_payment_data').addClass('hide');
        __write_number($('input#in_balance_due'), bal_due);
...
function get_contact_due(id) {
    $.ajax({
        method: 'get',
        url: '/get-contact-due-balance/' + id,
        dataType: 'json',
        success: function(result) {
            if (result) {
                $('.contact_due_text').find('span').text(result.output);
                $('.contact_due_text').removeClass('hide');

                __write_number($('input#contact_due_input'), result.output);


               

                var numericValue = parseFloat(result.output.replace(/[^\d.-]/g, ''));
                console.log(numericValue);
                if (numericValue > 0) {
                    // If total is less than or equal to 0, update styling
                    $('.contact_due_text').removeClass('text-green').addClass('text-danger');
                } else {
                    // If total is greater than 0, reset styling to default
                    $('.contact_due_text').removeClass('text-danger').addClass('text-green');
                }
                $('.contact_garantue_text').find('span').text(result.output1);
                $('.contact_garantue_text').removeClass('hide');

                $('.contact_total_text').find('span').text(result.total);
                $('.contact_total_text').removeClass('hide');
                 
                if (result.output > result.output1) {
                    // If total is less than or equal to 0, update styling
                    $('.contact_total_text').find('span').removeClass('text-danger').addClass('text-green');
                    $('.contact_total_text').find('strong').removeClass('text-danger').addClass('text-green');
                } else {
                    // If total is greater than 0, reset styling to default
                    $('.contact_total_text').find('span').removeClass('text-green').addClass('text-danger');
                }
                ...
                $('.contact_garantue_text').find('span').text('');
                $('.contact_garantue_text').addClass('hide');

                $('.contact_total_text').find('span').text('');
                $('.contact_total_text').addClass('hide');
            }
        },
    });
}
